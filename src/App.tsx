...
import { 
  Zap, ListTodo, Calendar, Bot, User, ShieldAlert, Sparkles, Award, LogOut
} from "lucide-react";
import { motion, AnimatePresence } from "motion/react";

type ActiveTab = "landing" | "auth" | "dashboard" | "planner" | "coach" | "settings";
...
export default function App() {
  ...
  // If user is already logged in on mount, go straight to dashboard
  useEffect(() => {
    if (currentUser) {
  ...
  // Authenticated fetch helper — dynamically reads currentUser
  const fetchWithAuth = async (url: string, options: RequestInit = {}) => {
    const user = currentUser || localStorage.getItem("aipilot_user");
    const headers = {
      ...(options.headers || {}),
    } as any;
    if (user) {
  ...
  // --- Render Auth or Landing ---
  if (activeTab === "landing") {
    return <LandingView onLogin={handleGoToLogin} onSignup={handleGoToSignup} />;
  }
  ...
  return (
    <div id="app-wrapper" className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
      ...
      <header id="app-header" className="sticky top-0 z-40 bg-slate-950/90 backdrop-blur-md border-b border-slate-900 px-6 py-4 flex items-center justify-between">
        <div id="app-logo" className="flex items-center gap-2 cursor-pointer" onClick={() => setActiveTab("dashboard")}>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <button
            ...
            title="Sign out"
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-900 border border-slate-800 text-xs font-sans font-semibold text-slate-400 hover:text-rose-400 hover:border-rose-900/50 transition-all cursor-pointer"
          >
